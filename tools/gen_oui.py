...
CURATED: list[tuple[str, list[str]]] = [
    ...
    # Wi-Fi and Ethernet module makers. These turn up constantly as the OUI of
    # a finished product whose own brand never registered a block: smart
    # heating, 3D printers, cameras, TVs and set-top boxes.
    ("FN-Link",          [r"^fn-link"]),
    ("Arcadyan",         [r"^arcadyan"]),
    ("Invensys",         [r"^invensys"]),
    ("Smart Innovation", [r"^smart innovation"]),
    ("Proxmox",          [r"^proxmox"]),
    ("Hewlett Packard",  [r"^hewlett packard", r"^hp inc", r"^hewlett-packard"]),
    ("Sagemcom",         [r"^sagemcom"]),
    ("Technicolor",      [r"^technicolor"]),
    ("Wistron",          [r"^wistron"]),
    ("Compal",           [r"^compal"]),
    ("Quanta",           [r"^quanta"]),
    ("Pegatron",         [r"^pegatron"]),
    ("Foxconn",          [r"^hon hai", r"^foxconn"]),
    ("Elitegroup",       [r"^elitegroup"]),
    ("Gigabyte",         [r"^giga-byte", r"^gigabyte"]),
    ("ASRock",           [r"^asrock"]),
    ("Micro-Star",       [r"^micro-star"]),
    ("Zyxel",            [r"^zyxel"]),
    ("Draytek",          [r"^draytek"]),
    ("Sercomm",          [r"^sercomm"]),
    ("Askey",            [r"^askey"]),
    ...
    ("Prusa",            [r"^prusa"]),
    ("Bambu Lab",        [r"^bambu"]),
    ("Creality",         [r"^creality", r"^shenzhen creality"]),
    ("Schneider",        [r"^schneider electric"]),
    ("Drayton",          [r"^drayton"]),
    ("Honor",            [r"^honor device"]),
    ("Huawei",           [r"^huawei"]),
    ("Oppo",             [r"^oppo", r"^guangdong oppo"]),
    ("Vivo",             [r"^vivo mobile"]),
    ("Motorola",         [r"^motorola"]),
    ("Nokia",            [r"^nokia"]),
    ("Garmin",           [r"^garmin"]),
    ...
    ("LIFX",             [r"^lifx"]),
    ("Nanoleaf",         [r"^nanoleaf"]),
    ("Aqara",            [r"^aqara", r"^lumi united"]),
    ("SmartThings",      [r"^smartthings"]),
    ("Denon",            [r"^denon", r"^d&m holdings"]),
    ("Yamaha",           [r"^yamaha"]),
    ("Bose",             [r"^bose"]),
    ("Sky",              [r"^sky uk", r"^british sky"]),
    ("Virgin Media",     [r"^virgin media"]),
    ("BT",               [r"^bt plc", r"^british telecom"]),
    ("IEEE Registration", [r"^ieee registration"]),
]
